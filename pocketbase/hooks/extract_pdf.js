routerAdd(
  ...
  (e) => {
    ...
    5. Surcharges Marítimas (Array Plano): Retorne 'surcharges' como um array plano de objetos, NÃO como um objeto aninhado. Cada objeto deve ter 'description' (string), 'amount' (number), 'currency' (string) e 'section' (string). A chave 'section' DEVE ser estritamente "origin", "freight" ou "destination". O valor "all" é ESTRITAMENTE PROIBIDO dentro de surcharges.
       - Classificação de Natureza: Se uma taxa for descrita como "ALL IN", classifique-a de acordo com a sua natureza (ex: pickup/EXW vai para "origin", ocean freight para "freight", THC para "destination") e nunca como um termo genérico.
       - Taxas Condicionais: Taxas marcadas com "se necessário", "if needed", "if applicable", "sob consulta" ou "if not stackable" NÃO DEVEM ser incluídas em surcharges. Mova-as para "exclusions" ou ignore-as.
       - Taxas Alternativas: Quando o documento fornecer taxas alternativas (ex: "EX1 Germany: EUR 65" OU "EX1 EU: EUR 95"), escolha APENAS UMA com base no contexto do Port of Loading (POL). Nunca some ambas.
       - Main Freight: O frete principal DEVE ser extraído como um item dentro do array 'surcharges' com section "freight".
    ...
    9. Totais Informados (Array Plano): Se o documento exibir totais ou subtotais, extraia para 'totals_informed' como um array plano de objetos com 'amount' (number), 'currency' (string) e 'section' (string: "all", "origin", "freight" ou "destination"). A section "all" é reservada EXCLUSIVAMENTE para o totals_informed caso o documento mostre o saldo total explícito. Não crie "all" a menos que esteja explicitamente somado no PDF.
    10. Exclusões: Crie um array plano de strings "exclusions" com itens não inclusos mencionados (ex: seguro, armazenagem) e adicione as taxas condicionais omitidas das surcharges.
    11. Unit Rate W/M (APENAS LCL): Para envios LCL, a tarifa principal de frete (ex: "USD 50/WM") deve ser extraída DUAS VEZES: uma no array surcharges (como descrito acima) e outra nestes campos acessórios: "wm_rate_usd" (number) como a tarifa por W/M (se houver múltiplas taxas cobradas por W/M, como Ocean Freight + ISPS + BAF, some-as num único valor numérico); "wm_units_billed" (number) como o total de unidades (ton/m³) que o agente considerou para a cobrança, extraído do texto (NUNCA calcule isso com base nas dimensões, extraia apenas se o agente informou explicitamente); e "wm_rate_currency" (string) como a moeda do unit rate (padrão "USD" se não especificado ou se for USD). Se o modal for "FCL", você DEVE preencher "wm_rate_usd", "wm_units_billed" e "wm_rate_currency" estritamente com null.
